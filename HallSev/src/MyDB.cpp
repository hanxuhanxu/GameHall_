...
{//wenti
	...
	redisReply *reply;
	reply =(redisReply*) redisCommand(conn, "SADD GameList  %s", GameNum);
	if(reply==NULL)
		return 0;
...
//	strcpy(str1,GameStr);
	...
	printf("add str??? %s\n",s);
	redisReply *re;
	re =(redisReply*) redisCommand(conn, "SET %s %s",GameNum, s);
	if(re==NULL)
		return 0;
	freeReplyObject(re);
	return 1;
...
{
	redisReply*reply = (redisReply*)redisCommand(conn, "SREM GameList %s", GameNum);
		
	freeReplyObject(reply);
	redisReply*re =(redisReply*)redisCommand(conn,"DEL %s",GameNum);
	freeReplyObject(re);
	return 1;
}
...
int CMyDB::GetGameList(vector<Game>&GameList)
{
	...
	redisReply*	reply = (redisReply*)redisCommand(conn,"SMEMBERS GameList");
...
int CMyDB::GetScore(char* id,char*GameNum){
	...
	redisReply*reply = (redisReply*)redisCommand(conn, "HGET %s %s",id, GameNum);
	int i;
	i=atoi(reply->str);
	freeReplyObject(reply);
	return i;	
...
int CMyDB::UpdateScore(char*id,char*gamenum,int score){
	...
	redisReply*reply = (redisReply*)redisCommand(conn, "HSET %s %s %d",id, gamenum,score);
	freeReplyObject(reply);
return 1;
}
